...
When a Task tool block is linked to a subagent session, it
shows an expandable toggle. Click it to view the subagent's
full conversation inline without leaving the parent session.
Messages load on demand when the section is expanded.
