...
agentsview auto-discovers session files from three agent types:
...
Change detection uses file content hashing (not just mtime)
to avoid unnecessary re-parsing. A pool of 8 workers processes
files in parallel during sync.

Files that fail to parse are tracked and retried when their
mtime changes.
