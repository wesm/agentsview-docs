...
Build from source (requires Go 1.25+ with CGO):

```bash
CGO_ENABLED=1 go install -tags fts5 \
  github.com/wesm/agentsview/cmd/agentsview@latest
```
...
2. Discover and sync sessions from Claude Code, Codex, and Gemini CLI
